...
const addDoctor = async (req, res) => {
    ...
    try {
        ...
        // Validate image file is uploaded
        if (!imageFile) {
        ...
        if (isNaN(fees) || Number(fees) <= 0) {
            return res.json({ success: false, message: "Please enter a valid fees amount" })
        }
        ...
        if (error.name === 'ValidationError') {
            ...
            return res.json({ success: false, message: `Validation Error: ${validationErrors.join(', ')}` })
        }
